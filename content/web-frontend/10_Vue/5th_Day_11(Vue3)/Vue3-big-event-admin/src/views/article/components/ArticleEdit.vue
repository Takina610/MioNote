...
const onSelectFile = (file) => {
  ...
  // 立刻将图片对象，存入 formModel.value.cover_img 用来提交
...
const open = async (params) => {
  ...
  if (params.id) {
    ...
    const res = await artGetDetailService(params.id)
    formModel.value = res.data.data
    // 图片需要单独处理回显
    imgUrl.value = baseURL + formModel.value.cover_img
    // 注意：提交给后台，需要的数据格式，是 file对象格式
    // 需要将网络图片地址 => 转换成 file对象，存储起来
    const file = await imageUrlToFile(imgUrl.value, formModel.value.cover_img)
    formModel.value.cover_img = file
  } else {
    formModel.value = {...defaultForm} // 基于默认的数据，重置 form的数据
    // 这里重置了表单的数据，但是图片上传 img地址，富文本编辑器的内容还没有重置 => 需要手动重置
    imgUrl.value = ''
