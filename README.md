...
| 收起 / 展开侧栏 | 侧栏左上角的箭头，或 `Ctrl + B`；收起后左下角有开关（整条侧栏往下沉进那个角落） |
| 调整侧栏宽度 | 拖侧栏右边缘；双击恢复默认宽度 |
| 收起 / 展开右侧目录 | 目录标题行右端的箭头；收起后右侧中间浮出一个 `<` 开关 |
| 打开上一篇 / 下一篇 | 笔记底部的按钮，按文件名编号顺序（带方向化的翻页过渡） |
...
src/lib/routeTransition.ts 上一篇/下一篇的换页过渡（View Transition + 方向）
...
- **收起类动画里，内容要等动画播完才隐藏**（`visibility` 延迟一个时长，展开那侧把延迟归零）。
  侧栏、右侧目录、文件树、笔记小节都是这条规矩；直接写 `visibility: hidden` 会变成"内容先闪没、再空缩"。
- **面板收起时，宽度要安排在内容的"下沉"之后**（`--ease-in-out` 留给宽度，`--expand-ease`
  留给下沉）。宽度前段发力的话，面板 70ms 就窄到看不见内容了——下沉做得再大也白搭。
- **面板宽度动的是栅格轨道，不是面板自己的 `width`**。`auto` 轨道按子项的**目标**尺寸算，
  不看过渡中的值，结果就是"跳"过去。侧栏动 `.app` 的 `grid-template-columns`，目录动 `.view` 的。
- **`var()` 拼错不会报错，会让整条声明静默失效**（回落成初值）。写新曲线/时长前先确认 token
  在 `:root` 里存在；判据是 `getComputedStyle(el).transitionProperty === 'all'` 且
  `transitionDuration === '0s'`。
- **`view-transition-name` 只在换页那 220ms 里挂上**（CSS 里由 `html[data-nav=…]` 决定）。
  它要求全局唯一，而换主题那套动画拍的是整页快照——正文列一旦有名字，圆形扩散会缺一块。
  同理，换页时不能同时再起一个 View Transition（`isPageTransitionRunning` 就是防这个的）。
- **上一篇/下一篇要预取上下篇**（`preloadNote`）。过渡在按下的一瞬间拍"新快照"，
  内容要是还在路上，拍到的就是"正在读取笔记…"，动画等于白做。
- **路由变化要同步通知**（`navigate()` 里那次 `subscribeRoute`）+ 过渡回调里 `flushSync`：
  `hashchange` 是异步派发的，不这么做浏览器拍到的新快照还是旧那一页。
